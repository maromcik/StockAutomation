...
public static class HoldingSnapshotLineParser
{
    ...
    private static IEnumerable<HoldingSnapshotLine> ParseLines(Stream stream)
    {
        ...
        while (!lineParser.EndOfData)
        {
            ...
            if (!DateTime.TryParseExact(fields[0], "MM/dd/yyyy", CultureInfo.InvariantCulture, DateTimeStyles.None, out var date)) yield break;
            var fund = fields[1];
            ...
            var marketValueUsd = decimal.Parse(fields[6].Replace("$", "").Replace(",", ""), CultureInfo.InvariantCulture);
